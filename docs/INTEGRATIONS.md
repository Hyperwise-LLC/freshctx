...
This integration was introduced in FreshCtx 0.9.0. Install the current release from PyPI with `python -m pip install 'freshctx[mcp-guard]==0.14.0'`. The dependency resolver receives only the MCP tool name. Tool arguments remain in the MCP SDK and are not copied into FreshCtx metadata. Authentication, authorization, transactions, retries, idempotency, and the correctness of the declared dependency map remain application responsibilities.
